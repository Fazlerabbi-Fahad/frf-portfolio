...
const BlogDetailPage = lazy(() => import("@/features/blog/BlogDetailPage").then((m) => ({ default: m.BlogDetailPage })));
const GalleryPage = lazy(() => import("@/features/gallery/GalleryPage").then((m) => ({ default: m.GalleryPage })));
const GalleryDetailPage = lazy(() => import("@/features/gallery/GalleryDetailPage").then((m) => ({ default: m.GalleryDetailPage })));
...
const Loading = () => <div className="py-32 text-center font-mono text-sm text-ash">Loading…</div>;
      ...
      <Suspense fallback={<Loading />}>
        <AnimatePresence mode="wait">
          <Routes location={location} key={location.pathname}>
            ...
            <Route path="/projects/:slug" element={<Page><ProjectDetailPage /></Page>} />
            <Route path="/blog" element={<Page><BlogPage /></Page>} />
            <Route path="/blog/:slug" element={<Page><BlogDetailPage /></Page>} />
            <Route path="/gallery" element={<Page><GalleryPage /></Page>} />
            <Route path="/gallery/:slug" element={<Page><GalleryDetailPage /></Page>} />
...
export default function App() {
  return (
    <Suspense fallback={<Loading />}>
      <Routes>
        <Route path="/admin/*" element={<div className="grain relative min-h-screen"><AdminApp /></div>} />
